...
codeunit 50607 "NXR Voice AI Service"
{
    ...
    begin
        ...
        EnhancedSystemPrompt += '- These are TEXT strings, NOT numeric values - thousand separators are NOT valid in Code fields\\';
        EnhancedSystemPrompt += '- ALWAYS remove ALL thousand separators: commas, periods, spaces, apostrophes, underscores\\';
        EnhancedSystemPrompt += '- "customer 10,000" → extract as "10000" (strip comma - US/UK format)\\';
        EnhancedSystemPrompt += '- "customer 10.000" → extract as "10000" (strip period - European format)\\';
        EnhancedSystemPrompt += '- "item 1 000" → extract as "1000" (strip spaces - French/Swedish format)\\';
        EnhancedSystemPrompt += '- "order 10''000" → extract as "10000" (strip apostrophe - Swiss format)\\';
        EnhancedSystemPrompt += '- "invoice 103_002" → extract as "103002" (strip underscore)\\';
        ...
        EnhancedSystemPrompt += '- Voice input may add separators for readability, but Code fields cannot contain them\\';
        EnhancedSystemPrompt += '\\';
        ...
        EnhancedSystemPrompt += '\\';
        EnhancedSystemPrompt += '\\\\MULTI-STEP QUERY GUIDANCE:\\';
        EnhancedSystemPrompt += '- When a query requires multiple steps (joins, complex relationships), execute the FIRST step and guide the user\\';
        EnhancedSystemPrompt += '- Use intent:"multistep" with step1Query (executable) and guidanceTemplate (text prompt with placeholders)\\';
        EnhancedSystemPrompt += '- Example: "Find vendor that sells the most critical items"\\';
        EnhancedSystemPrompt += '  {"intent":"multistep","step1Query":{"intent":"query","executionMode":"native","entity":"items","operation":"top","sortField":"quantityOnHand","limit":1},"guidanceTemplate":"Which vendor supplies item {itemNo}?","reason":"Query requires joining items and vendors"}\\';
        EnhancedSystemPrompt += '- Example: "Customers in cities where we have locations"\\';
        EnhancedSystemPrompt += '  {"intent":"multistep","step1Query":{"intent":"query","executionMode":"native","entity":"locations","operation":"list","limit":10},"guidanceTemplate":"Show customers in {locationCities}","reason":"Query requires cross-referencing locations and customers"}\\';
        EnhancedSystemPrompt += '- The guidanceTemplate can reference data from step1 results: {itemNo}, {customerName}, {locationCities}, etc.\\';
        EnhancedSystemPrompt += '- Always explain WHY multiple steps are needed\\';
        EnhancedSystemPrompt += '\\';
